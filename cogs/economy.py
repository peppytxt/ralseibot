...
class FishingLayout(ui.LayoutView):
    ...
    async def keep_callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("Essa vara não é sua! -w-", ephemeral=True)
...
class BaldeView(ui.View):
    ...
    async def vender_tudo(self, interaction: discord.Interaction, button: ui.Button):
        if interaction.user.id != self.user.id:
            return await interaction.response.send_message("Este balde não é seu! -w-", ephemeral=True)
...
class Economy(commands.Cog):
    ...
    async def check_economy_achievements(self, user_id: int, ctx_or_interaction=None):
            user_data = await self.col.find_one({"_id": user_id}) or {}
            coins = user_data.get("coins", 0)

            ach_cog = self.bot.get_cog("AchievementsCog")
            if ach_cog and coins >= 100000:
                message_context = None
                if ctx_or_interaction:
                    if isinstance(ctx_or_interaction, discord.Interaction):
                        message_context = ctx_or_interaction.message
                    elif isinstance(ctx_or_interaction, commands.Context):
                        message_context = ctx_or_interaction.message

                await ach_cog.give_achievement(user_id, "coins_100000", message_context=message_context)
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        if user and "last_daily" in user:
            ...
            if last_daily == today:
                await self.check_economy_achievements(user_id, ctx_or_interaction=interaction)
    ...
    async def pay(
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "**Ambos precisam confirmar para concluir**"
            ),
